...
var TracePlugin = {
  ...
  afterInit: function(chart) {
    ...
    if (xScaleType !== 'linear' && xScaleType !== 'time' && xScaleType !== 'category' && xScaleType !== 'logarithmic' && xScaleType !== 'realtime') {
      return;
    }
  ...
  doZoom: function(chart, start, end) {
    const xAxisName = chart.config.options.scales.x ? 'x' : chart.config.options.scales.xAxes ? 'xAxes' : undefined;
    chart.config.options.scales[xAxisName];
    ...
    // notify delegate
    var beforeZoomCallback = helpers.valueOrDefault(chart.config.options.plugins.crosshair.callbacks ? chart.config.options.plugins.crosshair.callbacks.beforeZoom : undefined, defaultOptions.callbacks.beforeZoom);
